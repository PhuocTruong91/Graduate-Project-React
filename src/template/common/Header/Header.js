...
function Header(props) {
    ...
    userStore.subscribe(() => {
        if(userStore.getState()){
            sessionStorage.setItem('user', JSON.stringify(userStore.getState()));
    ...
    return (
        <div className="container">
            <div id="header" className={"header " + (props.type === 'home' ? 'on-top' : 'account')}>
                <div className="left">
                    <img src={logo}/>
                ...
                { sessionStorage.user ?  
                    <div className="user-info">
                        <p>{JSON.parse(sessionStorage.user).name}</p>
