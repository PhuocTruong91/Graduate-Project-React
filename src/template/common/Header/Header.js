...
function Header() {
    ...
    expectedItemStore.subscribe(() => {
        var listFavouriteTemp = expectedItemStore.getState();
        if(listFavouriteTemp){
            var temp = [];
            listFavouriteTemp.sort(function (a,b) {
                return a.createdAt > b.createdAt;
            })
            console.log(listFavouriteTemp)
            var tempList = listFavouriteTemp.filter(function (obj) {
                if (temp.includes(obj.idProduct)){
                    return null;
                }else{
                    temp.push(obj.idProduct)
                    return obj;
                }
                
            })
            tempList.sort(() => -1);
            setListFavourite(tempList);
        }
    })
                ...
                { isLogin ?  
                    <div className="user-info">
                        ...
                        <div className="remind">
                            ...
                            {listFavourite.length > 0 ? <p>{listFavourite.length}</p> : ''}
                        </div>
                    </div> : 
